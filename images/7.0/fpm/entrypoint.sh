...
EOF
		...
		if [ ${DOLI_AUTO_INSTALL} -eq 1]; then
			# Install with CURL
			curl -kL -H "Host: $domain" -X POST ${DOLI_URL_ROOT}/install/fileconf.php > /dev/null 2>&1
			curl -kL -H "Host: $domain" -X POST ${DOLI_URL_ROOT}/install/step1.php --data "testpost=ok&action=set&selectlang=fr_FR" > /dev/null 2>&1
			curl -kL -H "Host: $domain" -X POST ${DOLI_URL_ROOT}/install/step2.php --data "testpost=ok&action=set&dolibarr_main_db_character_set=latin1&dolibarr_main_db_collation=utf8_unicode_ci&selectlang=fr_FR" > /dev/null 2>&1
			curl -kL -H "Host: $domain" -X POST ${DOLI_URL_ROOT}/install/step4.php --data "testpost=ok&action=set&selectlang=fr_FR" > /dev/null 2>&1
			curl -kL -H "Host: $domain" -X POST ${DOLI_URL_ROOT}/install/step5.php --data "testpost=ok&action=set&selectlang=fr_FR&pass=${DOLI_ADMIN_PASSWORD}&pass_verif=${DOLI_ADMIN_PASSWORD}" > /dev/null 2>&1
