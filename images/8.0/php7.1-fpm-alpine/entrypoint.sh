...
EOF
		...
		if [ -n "$DOLI_AUTO_CONFIGURE" ]; then
			echo "Initializing Dolibarr automatic installation..."
			mkdir -p /var/www/logs/


			echo "Calling Dolibarr install step 2: database installation..."
			curl -kL -o /var/www/logs/install_step2.html -s -w "%{http_code}" \
				-X POST "http://localhost/install/step2.php" \
				...
				> /var/www/logs/install_step2.status

			if ! grep -q 200 /var/www/logs/install_step2.status; then 
				log 'Something went wrong during Dolibarr database installation. Check the logs at /var/www/logs/install_step2.html for details.' 2
				exit 1
			fi


			echo "Calling Dolibarr install step 5: administration account creation..."
			curl -kL -o /var/www/logs/install_step5.html -s -w "%{http_code}" \
				-X POST "http://localhost/install/step5.php" \
				--data "testpost=ok&action=set&selectlang=fr_FR&pass=${DOLIBARR_ADMIN_PWD}&pass_verif=${DOLIBARR_ADMIN_PWD}" \
				> /var/www/logs/install_step5.status

			if ! grep -q 200 /var/www/logs/install_step5.status; then 
				log 'Something went wrong during Dolibarr administration setup. Check the logs at /var/www/logs/install_step5.html for details.' 2
				exit 1
			fi
